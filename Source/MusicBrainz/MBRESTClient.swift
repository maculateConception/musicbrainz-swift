...
class MBRESTClient {
    ...
    ///
    private let standardHeaders: [String: String] = ["User-Agent": "\(appName)/\(appVersion) ( \(appContact) )"]
    ...
    ///
    func getCoverArt(forArtist artist: String, andReleaseTitle releaseTitle: String) throws -> NSImage? {
    ...
    ///
    func getCoverArt(forArtist artist: String, andRecordingTitle title: String) throws -> NSImage? {
        
        do {

            if let matchingReleases = try queryRecordings(artist: artist, recordingTitle: title),
               let releaseWithCoverArt = checkReleasesForCoverArt(matchingReleases) {
    ...
    ///
    private func queryReleases(artist: String, releaseTitle: String) throws -> [MBRelease]? {
        ...
        if let url = URL(string: "\(musicBrainzAPIBaseURL)/release?query=release:\(escapedReleaseTitleString)%20AND%20artistname:\(escapedArtistString)&fmt=json"),
           ...
           let releasesArr = mbDict["releases"] as? [NSDictionary] {
            ...
            return releasesArr.compactMap {MBRelease($0)}
        }
    ...
    /// - throws any error that was thrown while making the request.
    ///
    private func queryRecordings(artist: String, recordingTitle: String) throws -> [MBRelease]? {
        ...
        if let url = URL(string: "\(musicBrainzAPIBaseURL)/recording?query=recording:\(escapedRecordingTitleString)%20AND%20artistname:\(escapedArtistString)&fmt=json"),
           ...
           let recordingsArr = mbDict["recordings"] as? [NSDictionary] {
            ...
            return recordingsArr.compactMap {MBRecording($0)}.flatMap {$0.releases}
        }
    ...
    ///
    private func checkReleasesForCoverArt(_ releases: [MBRelease]) -> MBRelease? {
        ...
        for release in releases {
            ...
            do {
                ...
                if let url = URL(string: "\(musicBrainzAPIBaseURL)/release/\(release.id)?fmt=json"),
                   ...
                   let archive = MBCoverArtArchive(archiveDict),
    ...
    /// - throws any error that was thrown while making the request.
    ///
    private func getFrontCoverImage(release: MBRelease) throws -> NSImage? {
...
///
/// The name of this application (used in a request header for all requests sent to MusicBrainz). Used to idenfity this app to MusicBrainz.
///
fileprivate let appName = Bundle.main.infoDictionary?["CFBundleName"] as? String ?? "MusicBrainz-Swift"
...
///
fileprivate let appVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String ?? "1.0.0"
